        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900 dark:text-gray-100">
                ...
                <form wire:submit.prevent="save">
                    <!-- Template -->
                    <div class="mb-4">
                        <label for="audit_template_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Pilih Template Audit</label>
                        <select wire:model="audit_template_id" id="audit_template_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200">
                            <option value="">-- Pilih Template --</option>
                            @foreach ($templates as $template)
                                <option value="{{ $template->id }}">{{ $template->title }}</option>
                            @endforeach
                        </select>
                        @error('audit_template_id') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
